require 'o_c_logger'
namespace :updatedaily do
  desc "controls the running of parsing scripts that are intended to be run daily"

  task :rsync => :environment do
    begin
    system "sh /media/'My Password'/ruby/rails_projects/issues/bin/daily/govtrack-rsync.sh /media/'My Password'/ruby/rails_projects/issues/db"
    rescue Exception => e

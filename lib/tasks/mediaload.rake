require 'o_c_logger'
namespace :mediaload do
  desc "controls the running of parsing scripts from media that are intended to be run daily"

  task :rsync => :environment do
    begin
      system "sh #{Rails.root}/bin/daily/media-rsync.sh #{Rails.root}/db/ "
